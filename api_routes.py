...
@router.get("/chinese-api/brands")
async def legacy_get_brands(db: Session = Depends(get_db)):
    """Legacy endpoint for brand compatibility"""
    response = await get_brands(False, db)
    # Transform to match old Chinese API format
    return {
...
@router.get("/chinese-api/brands/{brand_id}/models")
async def legacy_get_models(brand_id: str, db: Session = Depends(get_db)):
    """Legacy endpoint for model compatibility"""
    response = await get_phone_models(brand_id, False, db)
    # Transform to match old Chinese API format
    return {
        "success": response["success"],
        "models": [
            {
                "id": model["chinese_model_id"],
                "mobile_model_id": model["chinese_model_id"],
                "name": model["name"],
                "mobile_model_name": model["name"],
                "price": model["price"],
                "stock": model["stock"],
